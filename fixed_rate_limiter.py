...
class RateLimiter:
    ...
    def __init__(self,
        ...
        # Backoff configuration (compatible with tests)
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    raise APIError(f"Request failed after {max_retries + 1} attempts: {e}")
    ...
    def get_current_rate_status(self) -> Dict[str, Dict]:
        ...
        for endpoint_type in self.rates:
            ...
            recent_requests = sum(1 for req_time, _ in history if req_time > cutoff_time)
            
            # Calculate current rate
            window_duration = min(self.rate_limit_window, 
    ...
    def wait_for_capacity(self, endpoint_type: str, required_capacity: float = 0.1) -> None:
        ...
        if capacity_used > (1.0 - required_capacity):
            ...
            if history:
                ...
                wait_time = max(0, self.rate_limit_window - (time.time() - oldest_request_time))
    ...
    def _classify_endpoint(self, url: str) -> str:
        ...
        if "/quick-search" in url_lower or "/searches" in url_lower:
            ...
            return "activate"
        elif "/assets" in url_lower and any(method in url_lower for method in ["download", "location"]):
            return "download"
        else:
            return "general"
    ...
    def _track_request(self, endpoint_type: str, start_time: float, duration: float) -> None:
        ...
        """
        history = self.request_history[endpoint_type]
        history.append((start_time, duration))
        
        # Limit history size to prevent memory growth
        max_history = self.rates.get(endpoint_type, 10) * self.rate_limit_window * 2
    ...
    def _parse_retry_after(self, response: requests.Response) -> float:
        ...
        Args:
            response: HTTP response with rate limit
    ...
    def _update_rate_limits_from_response(self, 
        ...
        Args:
            response: HTTP response from Planet API
        ...
        if limit_header:
            try:
                detected_limit = int(limit_header)
                self.detected_limits[endpoint_type] = detected_limit
                
                # Update our rate if detected limit is lower than configured
                if detected_limit < self.rates.get(endpoint_type, float('inf')):
                    logger.info(
                        f"Detected lower rate limit for {endpoint_type}: "
                        f"{detected_limit} (was {self.rates[endpoint_type]})"
                    )
                    self.rates[endpoint_type] = detected_limit
    ...
    def update_rate_limit(self, endpoint_type: str, new_limit: int) -> None:
        ...
        logger.info(f"Updated rate limit for {endpoint_type}: {old_limit} -> {new_limit}")
    ...
    def _make_request_with_retry(self, method: str, url: str, **kwargs) -> requests.Response:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if (response.status_code in self.retry_config["retry_statuses"] and 
                    attempt < max_retries):
