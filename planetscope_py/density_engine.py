...
class DensityConfig:
    ...
    method: Union[DensityMethod, str] = (
        DensityMethod.AUTO
...
class SpatialDensityEngine:
    ...
    def calculate_density(
        ...
        try:
            ...
            logger.error(
                f"Density calculation failed after {computation_time:.2f}s: {e}"
            )
    ...
    def _process_single_density(
        ...
        if method == DensityMethod.RASTERIZATION:
            return self._calculate_rasterization_density(
                scene_polygons, roi_poly, config, start_time
            )
    ...
    def _calculate_rasterization_density(
        self,
        scene_polygons: List[Polygon],
        roi_poly: Polygon,
        config: DensityConfig,
    ...
    def _calculate_adaptive_grid_density(
        ...
        try:
            ...
            logger.warning(
                "Adaptive grid module not available, falling back to rasterization"
            )
            return self._calculate_rasterization_density(
                scene_polygons, roi_poly, config, start_time
            )
            ...
            logger.error(
                f"Adaptive grid calculation failed: {e}, falling back to rasterization"
            )
            return self._calculate_rasterization_density(
                scene_polygons, roi_poly, config, start_time
            )
    ...
    def _merge_chunk_results(
        ...
        if chunk_results:
            ...
            logger.warning(
                "Chunk merging simplified - full mosaic implementation needed"
            )
            return first_result
        else:
    ...
    def export_density_geotiff(
        self, result: DensityResult, output_path: str, compress: str = "lzw"
    ) -> None:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"Stats: min={result.stats['min']}, max={result.stats['max']}, mean={result.stats['mean']:.2f}"
        )
