...
class PlanetScopeQuery:
    ...
    def search_scenes(self,
                    ...
                    **kwargs) -> Dict:
        """Search for Planet scenes based on spatiotemporal criteria with proper API call handling.
        
        Executes a search request to Planet's Data API with comprehensive error handling
        and result processing. Supports both dictionary and Shapely geometry inputs.
        ...
        Returns:
            Dict: Search results containing:
                - 'features': List of matching Planet scene features
        ...
        """
        try:
            logger.info(f"Executing search for {len(item_types or ['PSScene'])} item types")
            ...
            base_url = getattr(self.config, 'base_url', 'https://api.planet.com/data/v1')
            ...
            if response.status_code != 200:
                raise APIError(f"Search failed with status {response.status_code}: {response.text}")
            ...
            logger.info(f"Search completed: {search_stats.get('total_scenes', 0)} scenes found")
    ...
    def filter_scenes_by_quality(self,
                                scenes: List[Dict],
                                min_quality: float = 0.7,
                                max_cloud_cover: float = 0.2,
                                exclude_night: bool = True) -> List[Dict]:
        ...
        logger.info(f"Quality filtering: {len(filtered_scenes)}/{len(scenes)} scenes passed")
    ...
    def get_scene_previews(self, scene_ids: List[str]) -> Dict[str, str]:
        ...
        for scene_id in scene_ids:
            try:
                ...
                assets_url = f"{self.config.base_url}/item-types/PSScene/items/{scene_id}/assets"
    ...
    def _build_search_filter(self, geometry, start_date, end_date, cloud_cover_max=0.2, **kwargs):
        ...
        try:
            start_date_formatted, end_date_formatted = validate_date_range(start_date, end_date)
            logger.debug(f"Formatted dates - Start: {start_date_formatted}, End: {end_date_formatted}")
        except Exception as e:
