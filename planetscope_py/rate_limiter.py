...
class RateLimiter:
    ...
    def _track_request(self, endpoint_type, start_time, duration):
        ...
        try:
            ...
            self.request_history[endpoint_type].append((actual_start_time, actual_duration))
            
            # Cleanup old entries
            cutoff_time = actual_start_time - self.rate_limit_window
            while (self.request_history[endpoint_type] and 
                self.request_history[endpoint_type][0][0] < cutoff_time):
            ...
            try:
                ...
                for fmt in http_date_formats:
                    try:
                        ...
                        delay_seconds = (retry_time_utc - current_time_utc).total_seconds()
            ...
            if limit_header:
                try:
                    ...
                    if limit_value is not None:
                        ...
                        logger.debug(f"Detected rate limit for {endpoint_type}: {detected_limit}")
                        
                except (ValueError, TypeError) as e:
                    # Log the issue but don't crash - this often happens with Mock objects
                    logger.debug(f"Could not parse rate limit header '{limit_header}': {e}")
            ...
            if remaining_header:
                try:
                    ...
                    if remaining_value is not None:
                        ...
                        logger.debug(f"Rate limit remaining for {endpoint_type}: {remaining}")
                        
                except (ValueError, TypeError) as e:
                    logger.debug(f"Could not parse remaining header '{remaining_header}': {e}")
            ...
            if reset_header:
                try:
                    ...
                    if reset_value is not None:
                        ...
                        logger.debug(f"Rate limit reset time for {endpoint_type}: {reset_time}")
...
class RetryableSession:
    ...
    def _make_request_with_retry(self, method: str, url: str, **kwargs) -> requests.Response:
        ...
        for attempt in range(max_retries + 1):  # ✅ Now max_retries is guaranteed to be an integer
            try:
