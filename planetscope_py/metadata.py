...
class MetadataProcessor:
    ...
    def extract_scene_metadata(self, scene: Dict) -> Dict:
        ...
        try:
            ...
            if acquired:
                try:
                    acq_datetime = datetime.fromisoformat(acquired.replace('Z', '+00:00'))
                    metadata.update({
                        "acquired": acquired,
            ...
            if geometry:
                try:
                    ...
                    logger.warning(f"Error extracting geometry metadata for scene {metadata.get('scene_id', 'unknown')}: {e}")
            ...
            try:
                ...
                metadata["overall_quality"] = self._calculate_overall_quality(metadata["quality_scores"])
            except Exception as e:
                logger.warning(f"Error calculating quality scores for scene {metadata.get('scene_id', 'unknown')}: {e}")
            ...
            try:
                ...
                logger.warning(f"Error calculating derived metrics for scene {metadata.get('scene_id', 'unknown')}: {e}")
    ...
    def assess_coverage_quality(self, 
                               scenes: List[Dict],
                               target_geometry: Optional[Union[Dict, Polygon]] = None) -> Dict:
        ...
        coverage_stats = self._calculate_coverage_statistics(scene_metadata, target_geometry)
    ...
    def filter_by_metadata_criteria(self,
                                scenes: List[Dict],
        ...
        filter_stats["retention_rate"] = (
            ...
            if filter_stats["original_count"] > 0 else 0.0
        )
    ...
    def _extract_geometry_metadata(self, geometry: Dict) -> Dict:
        ...
        try:
            ...
            try:
                if len(bounds) >= 4:
                    ...
                    if (isinstance(width, (int, float)) and isinstance(height, (int, float)) 
                        and height != 0 and width >= 0 and height > 0):
            ...
            return {
                ...
                "perimeter_km": geom.length * 111.32 if hasattr(geom, 'length') else 0.0,
    ...
    def _calculate_coverage_statistics(self, 
                                     ...
                                     target_geometry: Optional[Union[Dict, Polygon]]) -> Dict:
        ...
        if target_geometry:
            try:
                ...
                if target_area > 0:
                    stats["coverage_efficiency"] = min(1.0, stats["unique_coverage_km2"] / target_area)
    ...
    def _analyze_quality_distribution(self, scene_metadata: List[Dict]) -> Dict:
        ...
        quality_stats = {
            "suitability_distribution": dict(suitability_counts)
        }
    ...
    def _analyze_spatial_distribution(self, scene_metadata: List[Dict]) -> Dict:
        ...
        for metadata in scene_metadata:
            ...
            bounds = metadata.get("bounds")
            if bounds and all(key in bounds for key in ["west", "east", "south", "north"]):
                bounds_list.append({
        ...
        if bounds_list:
            ...
            spatial_stats["extent"] = {
                ...
                "south": round(min(all_south), 8), # Southernmost point across all scenes
                "north": round(max(all_north), 8)  # Northernmost point across all scenes
            }
    ...
    def _check_metadata_criteria(self, metadata: Dict, criteria: Dict) -> Tuple[bool, str]:
    ...
    def _generate_coverage_recommendations(self,
        ...
        if not recommendations:
            recommendations.append("Scene collection meets quality and coverage requirements.")
    ...
    def _create_collection_overview(self, scene_metadata: List[Dict]) -> Dict:
        ...
        overview = {
            ...
            "unique_satellites": len(set(
                m.get("satellite_id") for m in scene_metadata if m.get("satellite_id")
            )),
            "unique_item_types": len(set(
                m.get("item_type") for m in scene_metadata if m.get("item_type")
            ))
        }
