...
class PerformanceOptimizer:
    ...
    def __init__(self):
        ...
        logger.info(f"Performance optimizer initialized - {self.system_info.resource_level.value} resource system")
    ...
    def analyze_dataset(self,
                       ...
                       resolution: float) -> DatasetCharacteristics:
        ...
        logger.info(f"Dataset analysis: {roi_area_km2:.1f} km², {scene_count} scenes, "
                   f"{scene_density_per_km2:.1f} scenes/km², {estimated_raster_size_mb:.1f} MB raster")
        
        return characteristics
    
    def estimate_method_performance(self,
                                  method: str,
                                  dataset: DatasetCharacteristics) -> PerformanceEstimate:
        ...
        estimated_memory = (benchmarks["memory_per_megapixel"] * 
                          dataset.estimated_raster_size_mb / 4)  # Convert to megapixels
    ...
    def recommend_method(self,
                        ...
                        constraints: Optional[Dict] = None) -> Tuple[str, PerformanceEstimate]:
        """
        ...
        if profile == PerformanceProfile.SPEED:
            best_method = min(valid_estimates.keys(), 
                            key=lambda m: valid_estimates[m].estimated_time)
            ...
            best_method = max(valid_estimates.keys(),
                            key=lambda m: valid_estimates[m].accuracy_score)
        ...
        logger.info(f"Recommended method: {best_method} "
    ...
    def _apply_constraints(self,
                          ...
                          constraints: Dict) -> Dict[str, PerformanceEstimate]:
        ...
        for method, estimate in estimates.items():
            ...
            if "max_time" in constraints:
                if estimate.estimated_time > constraints["max_time"]:
                    logger.debug(f"{method} exceeds time constraint: "
            ...
            if "max_memory" in constraints:
                if estimate.estimated_memory > constraints["max_memory"]:
                    logger.debug(f"{method} exceeds memory constraint: "
            ...
            if "min_accuracy" in constraints:
                if estimate.accuracy_score < constraints["min_accuracy"]:
                    logger.debug(f"{method} below accuracy constraint: "
    ...
    def _auto_select_method(self,
                           dataset: DatasetCharacteristics,
                           estimates: Dict[str, PerformanceEstimate]) -> str:
        """Automatic method selection based on dataset characteristics."""
        
        # Rule-based selection
        if dataset.estimated_raster_size_mb > self.system_info.available_memory_gb * 1024 * 0.3:
            # Large raster - prefer adaptive grid or memory-efficient method
            memory_methods = sorted(estimates.keys(), 
                                  key=lambda m: estimates[m].estimated_memory)
    ...
    def _balanced_method_selection(self, estimates: Dict[str, PerformanceEstimate]) -> str:
        ...
        for method, estimate in estimates.items():
            ...
            time_score = 1.0 - (estimate.estimated_time / max_time) if max_time > 0 else 1.0
            memory_score = 1.0 - (estimate.estimated_memory / max_memory) if max_memory > 0 else 1.0
            accuracy_score = ((estimate.accuracy_score - min_accuracy) / accuracy_range 
                            if accuracy_range > 0 else 1.0)
            
            # Weighted combination (adjust weights as needed)
            combined_score = (0.3 * time_score + 
                            0.3 * memory_score + 
                            0.4 * accuracy_score)
    ...
    def optimize_parameters(self,
                           method: str,
                           dataset: DatasetCharacteristics) -> Dict[str, Any]:
    ...
    def _optimize_rasterization_params(self, dataset: DatasetCharacteristics) -> Dict[str, Any]:
        """Optimize parameters for rasterization method."""
        params = {}
        
        # Adjust chunk size based on memory
        available_memory_mb = self.system_info.available_memory_gb * 1024 * 0.5  # Use half
        
        if dataset.estimated_raster_size_mb > available_memory_mb:
            # Need chunking
            chunk_factor = math.ceil(dataset.estimated_raster_size_mb / available_memory_mb)
    ...
    def _optimize_vector_overlay_params(self, dataset: DatasetCharacteristics) -> Dict[str, Any]:
    ...
    def _optimize_adaptive_grid_params(self, dataset: DatasetCharacteristics) -> Dict[str, Any]:
    ...
    def record_performance(self,
                          ...
                          actual_time: float,
                          actual_memory: float,
                          quality_metrics: Optional[Dict] = None) -> None:
    ...
    def _update_benchmarks(self) -> None:
        ...
        for method, records in method_data.items():
            if len(records) >= 5 and method in self.method_benchmarks:
                ...
                avg_time_per_scene = np.mean([r['actual_time'] / r['dataset_size'] 
                                            for r in records if r['dataset_size'] > 0])
                ...
                logger.debug(f"Updated {method} benchmark: {avg_time_per_scene:.4f}s per scene")
        ...
        for method in method_usage.keys():
            ...
            if method_records:
                ...
                method_performance[method] = {
                    ...
                    'efficiency_score': 1.0 / (avg_time * avg_memory / 1000)  # Simple efficiency metric
                }
...
def integrate_optimizer(density_engine):
    ...
    def _enhanced_method_selection(self, scene_polygons, roi_poly, config):
        ...
        # Analyze dataset
        dataset = optimizer.analyze_dataset(mock_footprints, roi_poly, config.resolution)
        ...
        logger.info(f"Optimizer selected {selected_method.value} method "
                   f"(estimated: {estimate.estimated_time:.2f}s, {estimate.estimated_memory:.1f}MB)")
        
        return selected_method
    
    # Replace method selection
    density_engine._select_optimal_method = _enhanced_method_selection.__get__(density_engine)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Performance: {best_estimate.estimated_time:.2f}s, {best_estimate.estimated_memory:.1f}MB")
