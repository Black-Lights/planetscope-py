...
class DensityVisualizer:
    ...
    def plot_scene_footprints(self,
        ...
        roi_patch = MPLPolygon(roi_coords, fill=False, edgecolor='red', 
        ...
        for i, scene_poly in enumerate(scene_sample):
            try:
                ...
                patch = MPLPolygon(coords, fill=False, edgecolor='blue', 
    ...
    def create_summary_plot(self,
        ...
        im1 = ax1.imshow(masked_array, extent=extent, cmap='viridis', 
        ...
        if hasattr(density_result, 'stats') and 'error' not in density_result.stats:
            ...
            method_info = [
                ['Method', getattr(density_result, 'method_used', 'Unknown').value if hasattr(getattr(density_result, 'method_used', None), 'value') else str(getattr(density_result, 'method_used', 'Unknown'))],
                ['Time', f"{getattr(density_result, 'computation_time', 0):.2f}s"],
                ['Resolution', f"{density_result.grid_info.get('resolution', 'N/A')}m"],
                ['Grid Size', f"{density_result.grid_info.get('width', 0)}×{density_result.grid_info.get('height', 0)}"]
            ]
            ...
            ax3.text(0.5, 0.5, 'No statistics available', 
        ...
        if has_footprints:
            ...
            margin = max(bounds_roi[2] - bounds_roi[0], bounds_roi[3] - bounds_roi[1]) * 0.05
    ...
    def export_density_geotiff_with_style(self,
                                         density_result,
                                         output_path: str,
    ...
    def _get_safe_crs(self, preferred_crs: str) -> str:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"CRS {preferred_crs} failed, using fallback: {fallback_crs}")
                return fallback_crs
            except Exception:
                # Final fallback - no CRS
                logger.warning("CRS initialization failed, creating GeoTIFF without CRS")
                return None
    
    def _create_qgis_style_file(self,
                               density_result,
                               qml_path: str,
                               colormap: str) -> None:
...
def integrate_visualization(density_engine):
    ...
    def plot_result(self, result, output_dir=None, show_plots=True):
        ...
        # Create summary plot
        summary_path = os.path.join(output_dir, 'density_summary.png') if output_dir else None
...
if __name__ == "__main__":
    ...
    try:
        ...
        fig3 = visualizer.plot_scene_footprints(test_scenes, test_roi, title="Test Footprints")
