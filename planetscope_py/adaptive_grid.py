...
class AdaptiveGridEngine:
    ...
    def __init__(self, config: Optional[AdaptiveGridConfig] = None):
        ...
        logger.info(f"Adaptive grid engine initialized with base resolution {self.config.base_resolution}m")
    ...
    def calculate_adaptive_density(self,
        ...
        try:
            logger.info(f"Starting adaptive grid calculation for {len(scene_polygons)} scenes")
            ...
            logger.info(f"Adaptive grid calculation completed in {computation_time:.2f}s")
    ...
    def _create_initial_grid(self, roi_polygon: Polygon) -> List[GridCell]:
        ...
        cell_size_deg = self.config.base_resolution / 111000  # Convert meters to degrees
        ...
        for i in range(len(x_coords) - 1):
            for j in range(len(y_coords) - 1):
                cell_bounds = (x_coords[i], y_coords[j], x_coords[i+1], y_coords[j+1])
    ...
    def _calculate_initial_density(self, grid_cells: List[GridCell], scene_polygons: List[Polygon]) -> None:
    ...
    def _find_neighbor_cells(self, target_cell: GridCell, all_cells: List[GridCell]) -> List[GridCell]:
    ...
    def _iterative_refinement(self, initial_grid: List[GridCell], scene_polygons: List[Polygon]) -> List[GridCell]:
    ...
    def _identify_refinement_candidates(self, grid_cells: List[GridCell]) -> List[GridCell]:
        ...
        for cell in grid_cells:
            ...
            if self.config.criteria == RefinementCriteria.DENSITY_THRESHOLD:
                ...
                if (cell.density >= self.config.density_threshold or 
                    cell.variance >= self.config.variance_threshold):
                    needs_refinement = True
    ...
    def _refine_cell(self, parent_cell: GridCell, scene_polygons: List[Polygon], level: int) -> List[GridCell]:
    ...
    def _generate_density_array(self, final_grid: List[GridCell], roi_polygon: Polygon) -> Dict[str, Any]:
        ...
        output_resolution = self.config.base_resolution / (self.config.refinement_factor ** max_level)
        ...
        # Create transform
        transform = from_bounds(roi_bounds[0], roi_bounds[1], roi_bounds[2], roi_bounds[3], width, height)
        ...
        for level in range(max_level + 1):
            ...
            level_stats[f"level_{level}"] = {
                ...
                "resolution_m": self.config.base_resolution / (self.config.refinement_factor ** level),
                "avg_density": np.mean([cell.density for cell in level_cells]) if level_cells else 0
            }
...
def integrate_adaptive_grid(density_engine):
    ...
    def _calculate_adaptive_grid_density(self, scene_polygons, roi_poly, config, start_time):
    ...
    density_engine._calculate_adaptive_grid_density = _calculate_adaptive_grid_density.__get__(density_engine)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Density stats: min={result['stats']['min']}, max={result['stats']['max']}")
        
        # Show level statistics
        for level, stats in result['grid_info']['level_statistics'].items():
            print(f"{level}: {stats['cell_count']} cells at {stats['resolution_m']:.1f}m resolution")
