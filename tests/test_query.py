...
from planetscope_py.exceptions import (
    ValidationError, APIError, RateLimitError, PlanetScopeError
)
    ...
    with patch('planetscope_py.query.PlanetAuth') as mock_auth:
        ...
        with patch('planetscope_py.query.RateLimiter') as mock_rate_limiter_class:
            ...
            mock_rate_limiter._classify_endpoint.side_effect = lambda url: (
                "search" if any(endpoint in url.lower() for endpoint in ["/quick-search", "/searches", "/stats"])
                else "activate" if "activate" in url.lower()
                else "download" if any(endpoint in url.lower() for endpoint in ["/download", "/location"])
                else "general"
            )
...
class TestPlanetScopeQuery:
    ...
    def test_search_scenes_success_fixed(self, query_instance, sample_geometry, sample_search_response):
    ...
    def test_search_scenes_with_datetime_objects(self, query_instance, sample_geometry, sample_search_response):
    ...
    def test_search_scenes_network_error(self, query_instance, sample_geometry):
        ...
        query_instance.rate_limiter.make_request.side_effect = requests.exceptions.ConnectionError("Network error")
    ...
    def test_build_search_filter(self, query_instance, sample_geometry):
        ...
        geometry_filter = next(f for f in filter_components if f["type"] == "GeometryFilter")
        assert geometry_filter["field_name"] == "geometry"
        
        # Check date filter
        date_filter = next(f for f in filter_components if f["type"] == "DateRangeFilter")
        ...
        cloud_filter = next(f for f in filter_components if f["field_name"] == "cloud_cover")
    ...
    def test_build_search_filter_large_area(self, query_instance):
        ...
        large_geometry = {
            "type": "Polygon",
            "coordinates": [[
                [-130, 30], [130, 30], [130, 60], [-130, 60], [-130, 30]
            ]]
        }
    ...
    def test_batch_search_success(self, query_instance, sample_search_response):
        ...
        geometries = [
            ...
            },
            {
                "type": "Polygon",
                "coordinates": [[
                    [-122.41, 37.77], [-122.40, 37.77],
                    [-122.40, 37.78], [-122.41, 37.78], [-122.41, 37.77]
                ]]
            }
        ]
    ...
    def test_batch_search_with_failures(self, query_instance):
        ...
        geometries = [
            {"type": "Polygon", "coordinates": [[[-122.42, 37.77], [-122.41, 37.77], [-122.41, 37.78], [-122.42, 37.78], [-122.42, 37.77]]]},
            {"type": "Polygon", "coordinates": [[[-122.41, 37.77], [-122.40, 37.77], [-122.40, 37.78], [-122.41, 37.78], [-122.41, 37.77]]]}
        ]
    ...
    def test_classify_endpoint_fixed(self, query_instance):
        ...
        query_instance.rate_limiter._classify_endpoint = Mock(side_effect=lambda url: (
            "search" if any(endpoint in url.lower() for endpoint in ["/quick-search", "/searches", "/stats"])
            else "activate" if "activate" in url.lower()
            else "download" if any(endpoint in url.lower() for endpoint in ["/download", "/location"])
            else "general"
        ...
        assert query_instance.rate_limiter._classify_endpoint(
    ...
    def test_search_with_custom_item_types(self, query_instance, sample_geometry, sample_search_response):
    ...
    def test_search_with_additional_filters(self, query_instance, sample_geometry, sample_search_response):
        ...
        sun_filter = next(f for f in filter_config if f.get("field_name") == "sun_elevation")
        assert sun_filter["config"]["gte"] == 30.0
        
        # Check ground control filter
        gc_filter = next(f for f in filter_config if f.get("field_name") == "ground_control")
...
class TestRateLimitingIntegration:
    ...
    def test_rate_limit_429_handling_fixed(self, rate_limiter):
        ...
        with patch('time.sleep') as mock_sleep:
            response = rate_limiter.make_request("GET", "https://api.planet.com/data/v1/quick-search")
    ...
    def test_exponential_backoff_fixed(self, rate_limiter):
        ...
        with patch('time.sleep') as mock_sleep:
            response = rate_limiter.make_request("GET", "https://api.planet.com/data/v1/item-types")
...
class TestPerformanceAndEdgeCases:
    ...
    def test_search_with_extreme_date_range(self, query_instance, sample_geometry, sample_search_response):
        ...
        search_request = kwargs["json"]
        date_filter = next(
            f for f in search_request["filter"]["config"]
