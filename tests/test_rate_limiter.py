...
class TestRateLimiter:
    ...
    def test_classify_endpoint(self, rate_limiter):
        ...
        # Search endpoints
        assert rate_limiter._classify_endpoint(
    ...
    def test_parse_retry_after_http_date(self, rate_limiter):
        ...
        response.headers = {"Retry-After": retry_time.strftime("%a, %d %b %Y %H:%M:%S GMT")}
...
class TestRetryableSession:
    ...
    def test_request_with_retryable_status(self, retryable_session):
        ...
        fail_response.raise_for_status.side_effect = requests.HTTPError("Service unavailable")
    ...
    def test_should_retry_exception(self, retryable_session):
        ...
        assert retryable_session._should_retry_exception(ConnectionError("Connection failed"))
        ...
        assert not retryable_session._should_retry_exception(ValidationError("Invalid input"))
...
class TestIntegrationScenarios:
    ...
    def test_rate_limiting_with_retries(self):
        ...
        with patch('time.sleep') as mock_sleep:
            response = rate_limiter.make_request(
                "GET", 
                "https://api.planet.com/test/endpoint"
            )
    ...
    def test_concurrent_rate_limiting(self):
        ...
        def make_requests():
            ...
            try:
                for i in range(5):
                    start_time = time.time()
                    response = rate_limiter.make_request("GET", "https://api.planet.com/test")
                    end_time = time.time()
        ...
        assert len(error_list) == 0, f"Errors occurred during concurrent access: {error_list}"
        
        # All requests should complete
        assert len(request_times) == total_expected_requests, \
        ...
        assert all(0 <= t < 10.0 for t in request_times), \
        ...
        assert all(isinstance(t, (int, float)) for t in request_times), \
        ...
        print(f"   - Average request time: {sum(request_times) / len(request_times):.6f}s")
